...
{
    [TestFixture, Category("CC17349_NewAutoRegOOBA")]
    [Parallelizable(ParallelScope.Children)]
    class CC17349_NewAutoRegOOBA : BaseTestFixture
    {
        private const string className = "CC17349_NewAutoRegOOBA";
        public CC17349_NewAutoRegOOBA() : base(className, Properties.folderName, isHeadless: Properties.isHeadless) { }
        ...
        public static IEnumerable<TestCaseData> GetInputData()
        {
            foreach (var input in new GenericUtils().ReadInputData<InputData>(Properties.environment, className))
            {
                yield return new TestCaseData(input)
                    .SetName($"NewAutoRegOOBA{input.testSequence}");
            }
        }

        [Test, TestCaseSource(nameof(GetInputData))]
        [Author("Piyush Sharma")]
        public void NewAutoRegOOBA(InputData user)
        {
            /**************************************************************
             * 
             * Test:- New AutoReg Process with OOBA API
             * 
             * ************************************************************/
            ...
            Report.ChildLog = Report.ExtentTestGroup("Execute OOBA AutoReg API");
            ...
            string url = dBCreditCoach.ValidateExternalLead(externalLeadInfo, IdNumber, "ooba_new", "ooba");
            ...
            dBCreditCoach.ValidateLeadLog(LeadLogInfo, IdNumber, "ooba_new", "ooba");
            Report.PrintAndClearStep(Report.ChildLog);
        }
    }
}