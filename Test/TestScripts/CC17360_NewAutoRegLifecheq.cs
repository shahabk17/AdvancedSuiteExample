...
{
    [TestFixture, Category("CC17360_NewAutoRegLifecheq")]
    [Parallelizable(ParallelScope.Children)]
    class CC17360_NewAutoRegLifecheq : BaseTestFixture
    {
        private const string className = "CC17360_NewAutoRegLifecheq";
        public CC17360_NewAutoRegLifecheq() : base(className, Properties.folderName, isHeadless: Properties.isHeadless) { }
        ...
        public static IEnumerable<TestCaseData> GetInputData()
        {
            foreach (var input in new GenericUtils().ReadInputData<InputData>(Properties.environment, className))
            {
                yield return new TestCaseData(input)
                    .SetName($"NewAutoRegLifecheq{input.testSequence}");
            }
        }

        [Test, TestCaseSource(nameof(GetInputData))]
        [Author("Piyush Sharma")]
        public void NewAutoRegLifecheq(InputData user)
        {
            /**************************************************************
             * 
             * Test:- New AutoReg Process with Lifecheq API
             * 
             * ************************************************************/
            ...
            Report.ChildLog = Report.ExtentTestGroup("Execute Lifecheq AutoReg API");
            ...
            string url = dBCreditCoach.ValidateExternalLead(externalLeadInfo, IdNumber, "lifecheq", "lifecheq");
            ...
            dBCreditCoach.ValidateLeadLog(LeadLogInfo, IdNumber, "lifecheq", "lifecheq");
            Report.PrintAndClearStep(Report.ChildLog);
        }
    }
}