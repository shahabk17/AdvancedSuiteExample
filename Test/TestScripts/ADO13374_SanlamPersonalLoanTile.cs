...
{
    [TestFixture, Category("ADO13374_SanlamPersonalLoanTile")]
    [Parallelizable(ParallelScope.Children)]
    public class ADO13374_SanlamPersonalLoanTile : BaseTestFixture
    {
        private const string className = "ADO13374_SanlamPersonalLoanTile";
        public ADO13374_SanlamPersonalLoanTile() : base(className, Properties.folderName, isHeadless: Properties.isHeadless) { }
        ...
        public static IEnumerable<TestCaseData> GetInputData()
        {
                ...
                .ReadInputData<InputData>(Properties.environment, className))
            {
                yield return new TestCaseData(input)
                    .SetName($"SanlamPersonalLoanTile{input.testSequence}");
            }
        }

        [Test, TestCaseSource(nameof(GetInputData))]
        [Author("Shahab Khan")]
        public void SanlamPersonalLoanTile(InputData user)
        {
            /**************************************************************
             * 
             * Test:- Check For Qualifiers showing on the SPL tiles under
            ...
            Report.ChildLog = Report.ExtentTestGroup("Check for tile LMS and External Comm Log");
            solutionPageSteps.VerifySplTileButtonAndLogs(IdNumber, user.les_decision, bool.Parse(user.isQualifiedSPL));
            Report.PrintAndClearStep(Report.ChildLog);
            ...
            agentUiPageSteps.VerifySPLTileQualifiersOnAgentUi(IdNumber, bool.Parse(user.isQualifiedSPL));
            agentUiPageSteps.VerifyCommLogsOnAgentUi(agentUiPageSteps.expectedSplQualifier, "Transfer - SPL", bool.Parse(user.isQualifiedSPL));
            Report.PrintAndClearStep(Report.ChildLog);
        }
    }
}