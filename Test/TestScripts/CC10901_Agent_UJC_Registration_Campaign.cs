...
{
    [TestFixture, Category(nameof(CC10901_Agent_UJC_Registration_Campaign))]
    [Parallelizable(ParallelScope.Children)]
    class CC10901_Agent_UJC_Registration_Campaign : BaseTestFixture
    {
        private const string className = "CC10901_Agent_UJC_Registration_Campaign";

        public CC10901_Agent_UJC_Registration_Campaign() : base(className, Properties.folderName, isHeadless: Properties.isHeadless) { }
        ...
        public static IEnumerable<TestCaseData> GetInputData()
        {
                ...
                .ReadInputData<InputData>(Properties.environment, className))
            {
                yield return new TestCaseData(input)
                    .SetName($"Agent_UJC_RegistrationCampaign{input.testSequence}");
            }
        }

        [Test, TestCaseSource(nameof(GetInputData))]
        public void Agent_UJC_RegistrationCampaign(InputData user)
        {
            /**************************************************************
             * 
             * Test:- Check For UJC Registration Campaign on Agent UI 
             * with IdNumber and Landing Page URL set to Account Page
             *  
             * ************************************************************/
            ...
            var registrationDataset = agentUiPageSteps.CreateRegistrationCampaign_ContainIdNumber(true, true, false);
            Report.PrintAndClearStep(Report.ChildLog);

            Report.ChildLog = Report.ExtentTestGroup("Manage and Validate Registration Campaign");
            var ujc_updatedInfo = agentUiPageSteps.ValidateAndManageRegistrationCampaign(registrationDataset, true, true, false);
            Report.PrintAndClearStep(Report.ChildLog);
            ...
            agentUiPageSteps.ValidateCampaignOnDB(registrationDataset, ujc_updatedInfo, true, true);
            Report.PrintAndClearStep(Report.ChildLog);
        }
    }
}